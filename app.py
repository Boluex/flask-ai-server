#!/usr/bin/env python3
"""
TechFix AI Backend v2.1 - Upgraded for Agent v2.1
- Uses Mistral AI (your existing setup)
- Enhanced with log analysis + web research support
- All v1 security features preserved
- Compatible with agent v2.1 payload structure
"""
...
print("\n" + "="*60)
print("TECHFIX AI BACKEND v2.1 STARTUP")
print("="*60)
print(f"Environment: {os.getenv('FLASK_ENV', 'development')}")
print(f"SUPABASE_URL: {bool(SUPABASE_URL)}")
print(f"SUPABASE_KEY: {bool(SUPABASE_KEY)}")
print(f"MISTRAL_API_KEY: {bool(MISTRAL_API_KEY)}")
print(f"RESEND_API_KEY: {bool(RESEND_API_KEY)}")
print(f"TECHNICIAN_EMAIL: {TECHNICIAN_EMAIL}")
print("="*60 + "\n")


# ============= SECURITY FUNCTIONS (FROM V1) =============
...
def send_email_with_resend(to_email: str, subject: str, body: str):
    ...
    try:
        ...
        print(f"✅ [EMAIL SUCCESS] ID: {email.get('id', 'N/A')}")
        return True
...
def send_help_request_email(token: str, user_email: str, issue: str, rdp_code: str, complexity: int, attempts: int):
    """Enhanced with v2.1 context"""
    body = f"""
    <!DOCTYPE html>
    <html>
    <head>
        <style>
            ...
            .complexity {{ padding: 5px 10px; border-radius: 3px; display: inline-block; }}
            .complexity-high {{ background: #ff5252; color: white; }}
            .complexity-med {{ background: #ffa726; color: white; }}
            .complexity-low {{ background: #66bb6a; color: white; }}
        </style>
    </head>
    <body>
        <div class="container">
            <div class="header">
                <h2>🆘 Help Request - Agent v2.1</h2>
            </div>
            <div class="content">
                <div class="info-row">
                ...
                <div class="info-row">
                    ...
                    <span class="complexity {'complexity-high' if complexity >= 80 else 'complexity-med' if complexity >= 50 else 'complexity-low'}">
                        {complexity}/100
                    </span>
                </div>
                <div class="info-row">
                    <span class="label">AI Attempts:</span> {attempts}
                </div>
                <div class="info-row">
                    <span class="label">Remote Code:</span> 
                    <code style="background: #fff; padding: 5px 10px; border-radius: 3px;">{rdp_code}</code>
                </div>
    ...
    """
    send_email_async(TECHNICIAN_EMAIL, f"🆘 Help Request - {complexity}/100 complexity", body)


# ============= DATABASE FUNCTIONS (FROM V1) =============
...
# ============= AI FUNCTIONS (UPGRADED FOR V2.1) =============

def call_mistral_ai(prompt: str) -> dict:
    """Call Mistral AI with v2.1 enhanced prompt"""
    try:
        resp = requests.post(
            ...
            json={
                ...
                "temperature": 0.3,
                "max_tokens": 4000,  # Increased for v2.1
...
def build_enhanced_prompt_v2(payload: dict) -> str:
    """Build v2.1 enhanced prompt with logs + web research"""
    
    issue = payload.get("issue", "Unknown issue")
    logs = payload.get("logs", [])
    web_research = payload.get("web_research_summary", {})
    complexity = payload.get("complexity_score", 60)
    system_info = payload.get("system_info", {})
    
    # Extract top community solutions
    top_solutions = web_research.get("top_solutions", [])
    total_sources = web_research.get("total_sources", 0)
    
    prompt = f"""You are an expert system repair AI. Generate 3 ranked repair solutions in JSON format.
...
- CPUs: {system_info.get('cpu_count', 'Unknown')}

## COMPLEXITY ANALYSIS
- Score: {complexity}/100
- Tier: {"SAFE" if complexity < 50 else "MEDIUM" if complexity < 80 else "COMPLEX"}

## SYSTEM LOGS
"""

    if logs:
        prompt += f"Found {len(logs)} relevant log entries:\n"
        for i, log in enumerate(logs[:5], 1):
            msg = log.get("message", "")[:200]
            prompt += f"{i}. {msg}\n"
    else:
        prompt += "⚠️ No matching logs found.\n"
    
    prompt += f"\n## WEB RESEARCH ({total_sources} sources analyzed)\n"
    
    if top_solutions:
        prompt += "Community-validated solutions:\n\n"
        for i, sol in enumerate(top_solutions[:3], 1):
            prompt += f"{i}. **{sol['solution'].title()}**\n"
            prompt += f"   - Success: {sol['success_rate']:.0f}% ({sol['success_count']} reports)\n"
            prompt += f"   - Mentions: {sol['mentions']}\n\n"
    else:
        prompt += "No specific community solutions found.\n"
    
    prompt += """
## OUTPUT FORMAT (STRICT JSON)
Generate EXACTLY 3 solutions as a JSON object with "solutions" array:
  ...
  "solutions": [
    {
      ...
      "why_this_works": "Explanation citing logs + community data",
      "target_app": null,
      "needs_reboot": false
    }
  ]
...
## CRITICAL RULES
1. ALWAYS cite evidence from logs or community data
2. Prioritize solutions with high community success rates (>70%)
3. Commands must be executable (no placeholders)
4. Windows: Use `cmd` or `powershell` commands
5. Linux: Use `apt`, `yum`, `dnf` with `-y` flag
6. Rank by: community success > AI confidence > low risk
7. Set `needs_reboot: true` ONLY if absolutely necessary

Generate 3 solutions now (JSON only, no markdown):
"""
    
    return prompt


def sanitize_plan_v2(plan: dict, issue: str) -> dict:
    """Sanitize and validate v2.1 plan format"""
    ...
    if "error" in plan:
        return {
            "solutions": [{
                ...
                "steps": [{"description": plan["error"], "command": "", "requires_sudo": False}],
                "why_this_works": "AI encountered an error",
                "needs_reboot": False
            }]
        }
    
    # Validate structure
    if "solutions" not in plan or not isinstance(plan.get("solutions"), list):
        return {
            "solutions": [{
                ...
                "risk_level": 0,
                "estimated_time_minutes": 0,
                "steps": [{"description": "AI returned invalid format", "command": "", "requires_sudo": False}],
                "why_this_works": "Response parsing failed",
                "needs_reboot": False
            }]
        }
    ...
    sanitized_solutions = []
    for sol in plan["solutions"][:3]:  # Max 3
        sanitized_sol = {
            "title": str(sol.get("title", "Unnamed Solution"))[:200],
            "confidence": int(sol.get("confidence", 70)),
            "risk_level": int(sol.get("risk_level", 50)),
            "estimated_time_minutes": int(sol.get("estimated_time_minutes", 10)),
            "steps": [],
            "why_this_works": str(sol.get("why_this_works", ""))[:1000],
            "target_app": sol.get("target_app"),
            "needs_reboot": bool(sol.get("needs_reboot", False))
        }
        
        # Sanitize steps
        for step in sol.get("steps", [])[:10]:  # Max 10 steps per solution
            sanitized_sol["steps"].append({
                "description": str(step.get("description", ""))[:300],
                "command": str(step.get("command", ""))[:500],
                "requires_sudo": bool(step.get("requires_sudo", False))
            })
        
        if not sanitized_sol["steps"]:
            sanitized_sol["steps"] = [{
        ...
        sanitized_solutions.append(sanitized_sol)
    
    return {"solutions": sanitized_solutions}
...
@rate_limit
def generate_token():
    """Generate token (v1 logic preserved)"""
...
@rate_limit
def generate_plan():
    """Generate plan - UPGRADED FOR V2.1"""
    if request.method == 'OPTIONS':
        return '', 204
    ...
    try:
        ...
        # Build enhanced prompt with v2.1 context
        prompt = build_enhanced_prompt_v2(payload)
        
        print(f"\n[{datetime.now().isoformat()}] Generating plan...")
        print(f"Issue: {payload.get('issue', 'Unknown')[:50]}...")
        print(f"Complexity: {payload.get('complexity_score', 60)}/100")
        print(f"Logs: {len(payload.get('logs', []))}")
        print(f"Web sources: {payload.get('web_research_summary', {}).get('total_sources', 0)}")
        
        # Call Mistral
        raw_plan = call_mistral_ai(prompt)
        
        # Sanitize and validate
        plan = sanitize_plan_v2(raw_plan, payload.get('issue', 'Unknown'))
        
        # Add metadata
        plan["generated_at"] = datetime.now(timezone.utc).isoformat()
        plan["agent_version"] = "2.1"
        plan["complexity_score"] = payload.get("complexity_score", 60)
        
        # Save to Supabase
        supabase_update_session(token, {"plan": plan})
        
        solutions_count = len(plan.get("solutions", []))
        print(f"✅ Generated {solutions_count} solution(s)")
...
def request_human_help():
    """Request human help - UPGRADED FOR V2.1"""
    if request.method == 'OPTIONS':
        return '', 204
    ...
    try:
        ...
        complexity = int(data.get('complexity_score', 60))
        attempts = int(data.get('ai_attempts', 0))
        ...
        # Send enhanced email with v2.1 context
        send_help_request_email(token, email, issue, rdp_code, complexity, attempts)
...
def get_notifications():
    try:
        r = requests.get(
            ...
            headers=HEADERS,
            params={"select": "id,title,message,created_at", "order": "created_at.desc", "limit": 1},
            timeout=5
        )
...
def cleanup_old_sessions():
    try:
        ...
        return jsonify({"status": "success" if response.status_code in [200, 204] else "failed"}), 200
    except Exception as e:
...
if __name__ == '__main__':
    ...
    print("\n" + "="*60)
    print("   TECHFIX AI BACKEND v2.1 READY")
    print("="*60)
    print(f"Port: {port}")
    print(f"Debug: {debug}")
    print(f"AI Model: Mistral Small (mistral-small-latest)")
    print(f"Features: Log Analysis + Web Research + Community Data")
    print("="*60 + "\n")
